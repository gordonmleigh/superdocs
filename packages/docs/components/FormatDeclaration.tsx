import { getDeclarationUrlForName } from '@/util/getDeclarationUrl';
import {
  Declaration,
...
async function EntityName({ node }: NodeProps<ts.EntityName>) {
  const lib = await loadLibraryDefinition();
  const link = getDeclarationUrlForName(lib, node);
  const id: string[] = [];
